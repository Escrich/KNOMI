...
void MOONRAKER::get_printer_info(void) {
    ...
    if (!printer_info.isEmpty()) {
        ...
        //data.nozzle_actual = int16_t(json_parse["temperature"]["tool0"]["actual"].as<double>() + 0.5f); // Cambio en ambas lineas temporal, recibe la palabra Cuore en lugar de Tool0
        //data.nozzle_target = int16_t(json_parse["temperature"]["tool0"]["target"].as<double>() + 0.5f); // Escrich 20220905
        ...
        // Comprobación de Tool, es la que escribas en la web Knomi settings debajo de la IP y el puerto de la impresora, si no es correcto, no verás la temperatura
        //Serial.print("Tool que llega desde Moonraker: "); // Debe ser tool0
